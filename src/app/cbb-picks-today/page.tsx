...
export default function CBBPicksTodayPage() {
  ...
  // CBB can be labeled as CBB, NCAAB, or College Basketball
  const cbbPicks = topOverall.filter(p =>
    p.sport === 'CBB' ||
    p.sport === 'NCAAB' ||
    p.sport.toLowerCase().includes('college basketball')
  );
  const firePicks = cbbPicks.filter(p => p.capperCount >= 3);
    ...
    <div className="container px-4 py-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-2 text-primary mb-2">
              ...
              <span className="text-sm font-medium">College Basketball Picks</span>
            </div>
            <h1 className="text-3xl md:text-4xl font-bold mb-2">
              Free College Basketball Picks Today
            </h1>
            <p className="text-muted-foreground">
              {today} &middot; Expert consensus picks for today&apos;s CBB games
            </p>
          </div>
          <Button
            variant="outline"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <Card>
            <CardContent className="p-4">
              <div className="text-3xl font-bold text-primary">{cbbPicks.length}</div>
              <div className="text-sm text-muted-foreground">CBB Consensus Picks</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-3xl font-bold">
                {cbbPicks.length > 0 ? Math.max(...cbbPicks.map(p => p.capperCount)) : 0}
              </div>
      ...
      {isLoading && (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-2">Loading college basketball picks...</span>
        </div>
      )}
        ...
        <div className="space-y-4 mb-8">
          {cbbPicks.length > 0 ? (
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant="outline">CBB</Badge>
                          {pick.capperCount >= 3 && (
            ...
            <Card className="p-8 text-center">
              ...
              <p className="text-muted-foreground">No college basketball picks available today.</p>
              <p className="text-sm text-muted-foreground mt-2">CBB picks are available during basketball season (November - April).</p>
            </Card>
          )}
        </div>
      )}

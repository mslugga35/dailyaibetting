...
export default function NHLPicksTodayPage() {
  const { topOverall, isLoading, error, refetch, data } = useConsensus();

  const nhlPicks = topOverall.filter(p => p.sport === 'NHL');
  const firePicks = nhlPicks.filter(p => p.capperCount >= 3);
  ...
  return (
    <div className="container px-4 py-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-bold mb-2">
              Free NHL Picks Today
            </h1>
            <p className="text-muted-foreground">
              {today} &middot; Expert consensus picks for today&apos;s NHL games
            </p>
          </div>
          <Button
            variant="outline"
      ...
      {/* Sport Quick Nav */}
      <SportQuickNav currentSport="NHL" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <Card>
            <CardContent className="p-4">
              <div className="text-3xl font-bold text-primary">{nhlPicks.length}</div>
              <div className="text-sm text-muted-foreground">NHL Consensus Picks</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-3xl font-bold">
                {nhlPicks.length > 0 ? Math.max(...nhlPicks.map(p => p.capperCount)) : 0}
              </div>
      ...
      {isLoading && (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-2">Loading NHL picks...</span>
        </div>
      )}
        ...
        <div className="space-y-4 mb-8">
          {nhlPicks.length > 0 ? (
            ...
            <Card className="p-8 text-center">
              ...
              <p className="text-muted-foreground">No NHL picks available today.</p>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" asChild>
              <Link href="/mlb-picks-today">MLB Picks Today</Link>
            </Button>
            <Button variant="outline" asChild>

...
export default function NFLPicksTodayPage() {
  const { topOverall, isLoading, error, refetch, data } = useConsensus();

  const nflPicks = topOverall.filter(p => p.sport === 'NFL');
  const firePicks = nflPicks.filter(p => p.capperCount >= 3);
  ...
  return (
    <div className="container px-4 py-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-2 text-primary mb-2">
              ...
              <span className="text-sm font-medium">NFL Picks</span>
            </div>
            <h1 className="text-3xl md:text-4xl font-bold mb-2">
              Free NFL Picks Today
            </h1>
            <p className="text-muted-foreground">
              {today} &middot; Expert consensus picks for today&apos;s NFL games
            </p>
          </div>
          <Button
            variant="outline"
      ...
      {/* Sport Quick Nav */}
      <SportQuickNav currentSport="NFL" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <Card>
            <CardContent className="p-4">
              <div className="text-3xl font-bold text-primary">{nflPicks.length}</div>
              <div className="text-sm text-muted-foreground">NFL Consensus Picks</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-3xl font-bold">
                {nflPicks.length > 0 ? Math.max(...nflPicks.map(p => p.capperCount)) : 0}
              </div>
      ...
      {isLoading && (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-2">Loading NFL picks...</span>
        </div>
      )}
        ...
        <div className="space-y-4 mb-8">
          {nflPicks.length > 0 ? (
            ...
            <Card className="p-8 text-center">
              ...
              <p className="text-muted-foreground">No NFL picks available today.</p>

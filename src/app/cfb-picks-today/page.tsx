...
export default function CFBPicksTodayPage() {
  const { topOverall, isLoading, error, refetch, data } = useConsensus();

  // CFB can be labeled as CFB, NCAAF, or College Football
  const cfbPicks = topOverall.filter(p =>
    p.sport === 'CFB' ||
    p.sport === 'NCAAF' ||
    p.sport.toLowerCase().includes('college football')
  );
  const firePicks = cfbPicks.filter(p => p.capperCount >= 3);
  ...
  return (
    <div className="container px-4 py-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-2 text-primary mb-2">
              ...
              <span className="text-sm font-medium">College Football Picks</span>
            </div>
            <h1 className="text-3xl md:text-4xl font-bold mb-2">
              Free College Football Picks Today
            </h1>
            <p className="text-muted-foreground">
              {today} &middot; Expert consensus picks for today&apos;s CFB games
            </p>
          </div>
          <Button
            variant="outline"
      ...
      {/* Sport Quick Nav */}
      <SportQuickNav currentSport="CFB" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <Card>
            <CardContent className="p-4">
              <div className="text-3xl font-bold text-primary">{cfbPicks.length}</div>
              <div className="text-sm text-muted-foreground">CFB Consensus Picks</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-3xl font-bold">
                {cfbPicks.length > 0 ? Math.max(...cfbPicks.map(p => p.capperCount)) : 0}
              </div>
      ...
      {isLoading && (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-2">Loading college football picks...</span>
        </div>
      )}
        ...
        <div className="space-y-4 mb-8">
          {cfbPicks.length > 0 ? (
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant="outline">CFB</Badge>
                          {pick.capperCount >= 3 && (
            ...
            <Card className="p-8 text-center">
              ...
              <p className="text-muted-foreground">No college football picks available today.</p>
              <p className="text-sm text-muted-foreground mt-2">CFB picks are available during football season (August - January).</p>
            </Card>
          )}
        </div>
      )}
